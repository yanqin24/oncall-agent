...
后端 SHALL 为每个 Agent 工具调用保留一个 SQLite 审计条目，包含稳定的工具调用 ID、工具名称、JSON 安全参数、生命周期状态、有限的结果摘要、安全错误消息、开始时间戳、完成时间戳（可用时）以及派生的持续时间（以毫秒为单位）。
...
- **THEN** 后端 MUST 标记其审计记录 `failed`，存储一个安全的错误摘要，并保留其完成时间戳和持续时间。
...
- **WHEN** 调用者列出其拥有 user ID 的聊天会话的审计记录
- **THEN** 仓库 MUST 仅返回该 user 的会话记录

#### Scenario: Cross-tenant audit access is denied
- **WHEN** 当调用者尝试通过另一个 user 的聊天会话或诊断任务创建、更新或列出审核记录时
- **THEN** 仓库或 API MUST 应拒绝访问，而不暴露审核数据。
...
- **WHEN** 已认证的 user 打开他们包含已完成工具调用的聊天会话
