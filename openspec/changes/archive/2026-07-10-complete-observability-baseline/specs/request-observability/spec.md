...
后端 SHALL 会分配或保留请求相关标识符，将其作为 `X-Request-ID` 返回，并为请求完成和请求错误生成结构化的 JSON 生命周期日志，包含请求 ID、方法、路径、状态、延迟以及适用时的安全错误类别。
...
- **WHEN** 通过应用程序完成的请求
- **THEN** 的指标端点 MUST 会报告汇总的请求次数、失败次数和延迟数据，但不包含敏感的请求内容。

## ADDED Requirements

### Requirement: Correlated critical-work lifecycle logs
后端 SHALL 为文档索引、聊天/AIOps Agent 执行和本地 MCP 工具调用生成结构化的、安全的生命周期日志，当可用时使用活动请求 ID。
...
- **THEN** 后端 MUST 记录所属会话或诊断任务标识符、生命周期状态和持续时间，而不包括 user 消息内容、计划、工具参数或模型输出。
