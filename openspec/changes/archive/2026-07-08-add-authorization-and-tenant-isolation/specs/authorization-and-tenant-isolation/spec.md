...
#### Scenario: 创建块元数据
- **WHEN** 一份 chunk 文档已准备用于向量索引
- **THEN** 其元数据 MUST 包括 `ownerUserId`、`tenantId` 和 `knowledgeBaseId`。
...
前端 SHALL 仅从作用域限定于当前 user 的已认证 API 调用中渲染受保护的知识库、聊天和 AIOps 数据。

#### Scenario: Authenticated user views protected data
- **WHEN** 前端显示知识库、聊天或 AIOps 部分
- **THEN** 它 MUST 使用存储的认证令牌和当前-user 状态来请求作用域后端数据。

#### Scenario: User logs out
- **WHEN** 一个 user 注销
- **THEN** 前端 MUST 清除认证状态和受保护数据从可见的用户界面。
